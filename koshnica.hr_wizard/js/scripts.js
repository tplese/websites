...
$(window).on('load', function() {
    ...
    /*******************************/
    /* Resizing Step Wizard Window */
    /*******************************/

    var wizardPosition = 1;
    var heightOne = 117;
    var heightTwo = 70;
    var heightThree = 75;
    var heightFour = 75;
    ...
    $(".actions a[href$='#next']").click(function() {
        ...
        if (wizardPosition === 1)
        {
            $('.wizard > .content').css('min-height', heightOne + 'rem');
        }
        else if (wizardPosition === 2)
        {
            $('.wizard > .content').css('min-height', heightTwo + 'rem');
        }
        else if (wizardPosition === 3)
        {
            $('.wizard > .content').css('min-height', heightThree + 'rem');
        }
        else if (wizardPosition === 4)
        {
            $('.wizard > .content').css('min-height', heightFour + 'rem');
        }
        else if (wizardPosition === 5)
        {
            $('.wizard > .content').css('min-height', heightFive + 'rem');
        };
    });
    ...
    $("#add-job").click(function() {
        ...
        // Increases height of wizard window
        heightTwo += 48;
        $('.wizard > .content').css('min-height', heightTwo + 'rem');
        
        // Adds Title to each Job
        workDivIndex = 'work-' + workMainIndex + '-div-' + divIndex;
        var $workTitle = $('<div class="row divider-1 divider-left container sm-mt xs-mb"></div>');
        $workTitle.attr('id', workDivIndex);
        $("#work-experience").append($workTitle);
        var $titleSpan;

        if (workMainIndex == 2)
        {
            $titleSpan = $('<span class="xs-btn bg-colored3 white uppercase extrabold xs-madgin-y">Second last employment (Predzadnje zaposlenje)</span>');
        }
        else if (workMainIndex == 3)
        {
            $titleSpan = $('<span class="xs-btn bg-colored3 white uppercase extrabold xs-madgin-y">Third last employment (Pretpredzadnje zaposlenje)</span>');
        }
        else
        {
            $titleSpan = $('<span class="xs-btn bg-colored3 white uppercase extrabold xs-madgin-y">Previous employment (Prethodno zaposlenje)</span>');
        };
    
        $('#' + workDivIndex).append($titleSpan);

        // Adds From-To
        divIndex++;
        workDivIndex = 'work-' + workMainIndex + '-div-' + divIndex;
        var $workFromToDiv = $('<!-- From-To --><div class="row clearfix"><div class="col-md-8 col-sm-12 col-xs-12 t-left"></div></div>');
        $workFromToDiv.attr('id', workDivIndex);
        $("#work-experience").append($workFromToDiv);
            // adds label to From-To
            var $workFromToLabel = $('<label><b>From-To (Od-Do): </b><span style="color: red;">*</span></label>');
            $workFromToLabel.attr('for', workMainIndex + '-work-from-to');
            $('#' + workDivIndex + '> div').append($workFromToLabel);
            // adds input field to From-To
            var $workFromToInput = $('<input type="text" form="send-cv" placeholder="2017-present or 2016-2018" required class="classic_form big radius bs-hover bs-focus">');
            $workFromToInput.attr('id', workMainIndex + '-work-from-to');
            $workFromToInput.attr('name', workMainIndex + '_Work_From_To');
            $('#' + workDivIndex + '> div').append($workFromToInput);
        // Adds Employer
        divIndex++;
        workDivIndex = 'work-' + workMainIndex + '-div-' + divIndex;
        var $workEmployerDiv = $('<!-- Employer --><div class="row clearfix"><div class="col-md-8 col-sm-12 col-xs-12 t-left"></div></div>');
        $workEmployerDiv.attr('id', workDivIndex);
        $("#work-experience").append($workEmployerDiv);
            // adds label to Employer
            var $workEmployerLabel = $('<label><b>Employer (Poslodavac): </b><span style="color: red;">*</span></label>');
            $workEmployerLabel.attr('for', workMainIndex + '-work-employer');
            $('#' + workDivIndex + '> div').append($workEmployerLabel);
            // adds input field to Employer
            var $workEmployerInput = $('<input type="text" form="send-cv" placeholder="Podravka d.d." required class="classic_form big radius bs-hover bs-focus">');
            $workEmployerInput.attr('id', workMainIndex + '-work-employer');
            $workEmployerInput.attr('name', workMainIndex + '_Work_Employer');
            $('#' + workDivIndex + '> div').append($workEmployerInput);
        // Adds Country
        divIndex++;
        workDivIndex = 'work-' + workMainIndex + '-div-' + divIndex;
        var $workCountryDiv = $('<!-- Work - Country --><div class="row clearfix"><div class="col-md-8 col-sm-12 col-xs-12 t-left"></div></div>');
        $workCountryDiv.attr('id', workDivIndex);
        $("#work-experience").append($workCountryDiv);
            // adds label to Country
            var $workCountryLabel = $('<label><b>Country of Work (Zemlja rada): </b><span style="color: red;">*</span></label>');
            $workCountryLabel.attr('for', workMainIndex + '-work-country');
            $('#' + workDivIndex + '> div').append($workCountryLabel);
            // adds input field to Country
            var $workCountryInput = $('<input type="text" form="send-cv" placeholder="Croatia" required class="classic_form big radius bs-hover bs-focus">');
            $workCountryInput.attr('id', workMainIndex + '-work-country');
            $workCountryInput.attr('name', workMainIndex + '_Work_Country');
            $('#' + workDivIndex + '> div').append($workCountryInput);
        // Adds Row Position Name
        divIndex++;
        workDivIndex = 'work-' + workMainIndex + '-div-' + divIndex;
        var $workPositionDiv = $('<!-- Position --><div class="row clearfix"><div class="col-md-8 col-sm-12 col-xs-12 t-left"></div></div>');
        $workPositionDiv.attr('id', workDivIndex);
        $("#work-experience").append($workPositionDiv);
            // adds label to Position Name
            var $workPositionLabel = $('<label><b>Position name (Naziv radnog mjesta): </b><span style="color: red;">*</span></label>');
            $workPositionLabel.attr('for', workMainIndex + '-work-position');
            $('#' + workDivIndex + '> div').append($workPositionLabel);
            // adds input field to Position Name
            var $workPositionInput = $('<input type="text" form="send-cv" placeholder="Warehouse worker" required class="classic_form big radius bs-hover bs-focus">');
            $workPositionInput.attr('id', workMainIndex + '-work-position');
            $workPositionInput.attr('name', workMainIndex + '_Work_Position');
            $('#' + workDivIndex + '> div').append($workPositionInput);
        // Adds Row Job Description
        divIndex++;
        workDivIndex = 'work-' + workMainIndex + '-div-' + divIndex;
        var $workDescriptionDiv = $('<!-- Job description --><div class="row clearfix"><div class="col-md-8 col-sm-12 col-xs-12 t-left"></div></div>');
        $workDescriptionDiv.attr('id', workDivIndex);
        $("#work-experience").append($workDescriptionDiv);
            // adds label to Job Description
            var $workDescriptionLabel = $('<label><b>Main tasks (Opis glavnih zaduženja): </b></label>');
            $workDescriptionLabel.attr('for', workMainIndex + '-work-description');
            $('#' + workDivIndex + '> div').append($workDescriptionLabel);
            // adds textarea field Job Description
            var $workDescriptionArea = $('<textarea type="text" form="send-cv" placeholder="eg. Boxing products. Arranging products on palletes. Driving forklift. Following manager and forman orders..." class="classic_form big radius bs-hover bs-focus"></textarea>');
            $workDescriptionArea.attr('id', workMainIndex + '-work-description');
            $workDescriptionArea.attr('name', workMainIndex + '_Work_Description');
            $('#' + workDivIndex + '> div').append($workDescriptionArea);
    });
    ...
    var educationMainIndex = 1;
    
    $("#add-education").click(function() {
        var educationDivIndex;
        educationMainIndex++;
        var divIndex = 0;
        ...
        // Adds Title to each Education
        educationDivIndex = 'education-' + educationMainIndex + '-div-' + divIndex;
        var $educationTitle = $('<div class="row divider-1 divider-left container sm-mt xs-mb"></div>');
        $educationTitle.attr('id', educationDivIndex);
        $("#education").append($educationTitle);
        var $titleSpan;

        if (educationMainIndex == 2)
        {
            $titleSpan = $('<span class="xs-btn bg-colored3 white uppercase extrabold xs-madgin-y">Second last education (Predzadnje obrazovanje)</span>');
        }
        else if (educationMainIndex == 3)
        {
            $titleSpan = $('<span class="xs-btn bg-colored3 white uppercase extrabold xs-madgin-y">Third last education (Pretpredzadnje obrazovanje)</span>');
        }
        else
        {
            $titleSpan = $('<span class="xs-btn bg-colored3 white uppercase extrabold xs-madgin-y">Previous education (Prethodno obrazovanje)</span>');
        };
    
        $('#' + educationDivIndex).append($titleSpan);

        // Adds Education Type
        divIndex++;
        educationDivIndex = 'education-' + educationMainIndex + '-div-' + divIndex;
        var $educationTypeDiv = $('<div class="row clearfix"><div class="col-md-8 col-sm-12 col-xs-12 t-left"></div></div>');
        $educationTypeDiv.attr('id', educationDivIndex);
        $("#education").append($educationTypeDiv);
            // adds label to Eduction Type
            var $educationTypeLabel = $('<label><b>Formal education type (Vrsta formalnog obrazovanja): </b><span style="color: red;">*</span></label>');
            $educationTypeLabel.attr('for', educationMainIndex + '-education-type');
            $('#' + educationDivIndex + '> div').append($educationTypeLabel);
            // adds selection field to Eduction Type
            var $educationTypeSelect = $('<select form="send-cv" required class="classic_form big radius bs-hover bs-focus"><option value="">Select (Izaberi)</option><option value="Primary School">Primary School (Osnovna škola)</option><option value="Secondary: General (4 years)">Secondary general-education school (Gimnazija: 4 godine)</option><option value="Secondary: Vocational (4 years)">Secondary Vocational school (Strukovna srednja: 4 godine)</option><option value="Secondary: Vocational (3 years)">Secondary Vocational school (Strukovna srednja: 3 godine)</option><option value="Tertiary: Polytechnic school">Polytechnic school (Viša škola)</option><option value="Tertiary: University degree">University (Fakultet)</option><option value="Tertiary: Master Programme">Master (Magisterij)</option><option value="Tertiary: Doctoral Programme">Doctor (Doktorat)</option></select>');
            $educationTypeSelect.attr('id', educationMainIndex + '-education-type');
            $educationTypeSelect.attr('name', educationMainIndex + '_Education_Type');
            $('#' + educationDivIndex + '> div').append($educationTypeSelect);
        // Adds Education Institution
        divIndex++;
        educationDivIndex = 'education-' + educationMainIndex + '-div-' + divIndex;
        var $educationInstitutionDiv = $('<div class="row clearfix"><div class="col-md-8 col-sm-12 col-xs-12 t-left"></div></div>');
        $educationInstitutionDiv.attr('id', educationDivIndex);
        $("#education").append($educationInstitutionDiv);
            // adds label to Education Institution
            var $educationInstitutionLabel = $('<label><b>Education Institution Name (Ime obrazovne institucije): </b><span style="color: red;">*</span></label>');
            $educationInstitutionLabel.attr('for', educationMainIndex + '-education-institution');
            $('#' + educationDivIndex + '> div').append($educationInstitutionLabel);
            // adds selection field to Education Institution
            var $educationInstitutionInput = $('<input type="text" form="send-cv" placeholder="Electrical Engineering Vocational School Zagreb" class="classic_form big radius bs-hover bs-focus">');
            $educationInstitutionInput.attr('id', educationMainIndex + '-education-institution');
            $educationInstitutionInput.attr('name', educationMainIndex + '_Education_Institution');
            $('#' + educationDivIndex + '> div').append($educationInstitutionInput);
        // Adds Education Programme
        divIndex++;
        educationDivIndex = 'education-' + educationMainIndex + '-div-' + divIndex;
        var $educationProgrammeDiv = $('<div class="row clearfix"><div class="col-md-8 col-sm-12 col-xs-12 t-left"></div></div>');
        $educationProgrammeDiv.attr('id', educationDivIndex);
        $("#education").append($educationProgrammeDiv);
            // adds label to Education Programme
            var $educationProgrammeLabel = $('<label><b>Programme name (Ime smjera): </b><span style="color: red;">*</span></label>');
            $educationProgrammeLabel.attr('for', educationMainIndex + '-education-programme');
            $('#' + educationDivIndex + '> div').append($educationProgrammeLabel);
            // adds selection field to Education Programme
            var $educationProgrammeInput = $('<input type="text" form="send-cv" placeholder="Electrical engineering technician" class="classic_form big radius bs-hover bs-focus">');
            $educationProgrammeInput.attr('id', educationMainIndex + '-education-programme');
            $educationProgrammeInput.attr('name', educationMainIndex + '_Education_Programme');
            $('#' + educationDivIndex + '> div').append($educationProgrammeInput);
        // Adds Education Country
        divIndex++;
        educationDivIndex = 'education-' + educationMainIndex + '-div-' + divIndex;
        var $educationCountryDiv = $('<div class="row clearfix"><div class="col-md-8 col-sm-12 col-xs-12 t-left"></div></div>');
        $educationCountryDiv.attr('id', educationDivIndex);
        $("#education").append($educationCountryDiv);
            // adds label to Education Country
            var $educationCountryLabel = $('<label><b>Country of Education (Zemlja obrazovanja): </b><span style="color: red;">*</span></label>');
            $educationCountryLabel.attr('for', educationMainIndex + '-education-country');
            $('#' + educationDivIndex + '> div').append($educationCountryLabel);
            // adds selection field to Education Country
            var $educationCountryInput = $('<input type="text" form="send-cv" placeholder="Croatia" class="classic_form big radius bs-hover bs-focus">');
            $educationCountryInput.attr('id', educationMainIndex + '-education-country');
            $educationCountryInput.attr('name', educationMainIndex + '_Education_Country');
            $('#' + educationDivIndex + '> div').append($educationCountryInput);
        // Adds Education Year Completed
        divIndex++;
        educationDivIndex = 'education-' + educationMainIndex + '-div-' + divIndex;
        var $educationYearDiv = $('<div class="row clearfix"><div class="col-md-8 col-sm-12 col-xs-12 t-left"></div></div>');
        $educationYearDiv.attr('id', educationDivIndex);
        $("#education").append($educationYearDiv);
            // adds label to Education Year Completed
            var $educationYearLabel = $('<label><b>Year completed (Godina završetka): </b><span style="color: red;">*</span></label>');
            $educationYearLabel.attr('for', educationMainIndex + '-education-year-completed');
            $('#' + educationDivIndex + '> div').append($educationYearLabel);
            // adds selection field to Education Year Completed
            var $educationYearInput = $('<input type="text" form="send-cv" placeholder="2004" class="classic_form big radius bs-hover bs-focus">');
            $educationYearInput.attr('id', educationMainIndex + '-education-year-completed');
            $educationYearInput.attr('name', educationMainIndex + '_Education_Year_Completed');
            $('#' + educationDivIndex + '> div').append($educationYearInput);
    });
    ...
    $("#add-course").click(function() {
        ...
        // Adds Title to each Course
        courseDivIndex = 'course-' + courseMainIndex + '-div-' + divIndex;
        var $courseTitle = $('<div class="row divider-1 divider-left container sm-mt xs-mb"></div>');
        $courseTitle.attr('id', courseDivIndex);
        $("#course").append($courseTitle);
        var $titleSpan;

        if (courseMainIndex == 1)
        {
            $titleSpan = $('<span class="xs-btn bg-colored3 white uppercase extrabold xs-madgin-y">Latest course (Zadnji tečaj)</span>');
        }
        else if (courseMainIndex == 2)
        {
            $titleSpan = $('<span class="xs-btn bg-colored3 white uppercase extrabold xs-madgin-y">Second last course (Predzadnji tečaj)</span>');
        }
        else if (courseMainIndex == 3)
        {
            $titleSpan = $('<span class="xs-btn bg-colored3 white uppercase extrabold xs-madgin-y">Third last course (Pretpredzadnji tečaj)</span>');
        }
        else
        {
            $titleSpan = $('<span class="xs-btn bg-colored3 white uppercase extrabold xs-madgin-y">Previous course (Prethodni tečaj)</span>');
        };
    
        $('#' + courseDivIndex).append($titleSpan);

        // Adds Course Name
        divIndex++;
        courseDivIndex = 'course-' + courseMainIndex + '-div-' + divIndex;
        var $courseNameDiv = $('<div class="row clearfix"><div class="col-md-8 col-sm-12 col-xs-12 t-left"></div></div>');
        $courseNameDiv.attr('id', courseDivIndex);
        $("#course").append($courseNameDiv);
            // adds label to Course Name
            var $courseNameLabel = $('<label><b>Course name (Ime tečaja): </b><span style="color: red;">*</span></label>');
            $courseNameLabel.attr('for', courseMainIndex + '-course-name');
            $('#' + courseDivIndex + '> div').append($courseNameLabel);
            // adds selection field to Course Name
            var $courseNameInput = $('<input type="text" form="send-cv" placeholder="Safe Pass" class="classic_form big radius bs-hover bs-focus">');
            $courseNameInput.attr('id', courseMainIndex + '-course-name');
            $courseNameInput.attr('name', courseMainIndex + '_Course_Name');
            $('#' + courseDivIndex + '> div').append($courseNameInput);
        // Adds Course Provider
        divIndex++;
        courseDivIndex = 'course-' + courseMainIndex + '-div-' + divIndex;
        var $courseProviderDiv = $('<div class="row clearfix"><div class="col-md-8 col-sm-12 col-xs-12 t-left"></div></div>');
        $courseProviderDiv.attr('id', courseDivIndex);
        $("#course").append($courseProviderDiv);
            // adds label to Course Provider
            var $courseProviderLabel = $('<label><b>Course Provider (Pružatelj tečaja): </b><span style="color: red;">*</span></label>');
            $courseProviderLabel.attr('for', courseMainIndex + '-course-provider');
            $('#' + courseDivIndex + '> div').append($courseProviderLabel);
            // adds selection field to Course Provider
            var $courseProviderInput = $('<input type="text" form="send-cv" placeholder="Courses and trainings Limited" class="classic_form big radius bs-hover bs-focus">');
            $courseProviderInput.attr('id', courseMainIndex + '-course-provider');
            $courseProviderInput.attr('name', courseMainIndex + '_Course_Provider');
            $('#' + courseDivIndex + '> div').append($courseProviderInput);
        // Adds Course Country
        divIndex++;
        courseDivIndex = 'course-' + courseMainIndex + '-div-' + divIndex;
        var $courseCountryDiv = $('<div class="row clearfix"><div class="col-md-8 col-sm-12 col-xs-12 t-left"></div></div>');
        $courseCountryDiv.attr('id', courseDivIndex);
        $("#course").append($courseCountryDiv);
            // adds label to Course Country
            var $courseCountryLabel = $('<label><b>Country held (Zemlja održavanja): </b><span style="color: red;">*</span></label>');
            $courseCountryLabel.attr('for', courseMainIndex + '-course-country');
            $('#' + courseDivIndex + '> div').append($courseCountryLabel);
            // adds selection field to Course Country
            var $courseCountryInput = $('<input type="text" form="send-cv" placeholder="Croatia" class="classic_form big radius bs-hover bs-focus">');
            $courseCountryInput.attr('id', courseMainIndex + '-course-country');
            $courseCountryInput.attr('name', courseMainIndex + '_Course_Country');
            $('#' + courseDivIndex + '> div').append($courseCountryInput);
        // Adds Course Year Completed
        divIndex++;
        courseDivIndex = 'course-' + courseMainIndex + '-div-' + divIndex;
        var $courseYearDiv = $('<div class="row clearfix"><div class="col-md-8 col-sm-12 col-xs-12 t-left"></div></div>');
        $courseYearDiv.attr('id', courseDivIndex);
        $("#course").append($courseYearDiv);
            // adds label to Course Year Completed
            var $courseYearLabel = $('<label><b>Year Completed (Godina završetka): </b><span style="color: red;">*</span></label>');
            $courseYearLabel.attr('for', courseMainIndex + '-course-year-completed');
            $('#' + courseDivIndex + '> div').append($courseYearLabel);
            // adds selection field to Course Year Completed
            var $courseYearInput = $('<input type="text" form="send-cv" placeholder="2018" class="classic_form big radius bs-hover bs-focus">');
            $courseYearInput.attr('id', courseMainIndex + '-course-year-completed');
            $courseYearInput.attr('name', courseMainIndex + '_Course_Year_Completed');
            $('#' + courseDivIndex + '> div').append($courseYearInput);
    });
    ...
    $(function() {
        const scriptURL = 'https://script.google.com/macros/s/AKfycbynHRHTHF3f8Msi4LAWzay_xxa8S8FxDEUUHUW9-7HaUJGkPPk/exec';
